...
func TestMain(m *testing.M) {
	var err error
	db, err = xorm.NewEngine("mysql", "root:mysqlpw@tcp(127.0.0.1:55000)/test?charset=utf8mb4&parseTime=true&loc=Local")
	if err != nil {
		log.Fatalln(err)
	}
...
func mustInitDB(t *testing.T, db xorm.EngineInterface) {
	...
	_, err = session.Exec(`SET SESSION sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY,',''))`)
	if err != nil {
		log.Fatalln(err)
	}

	_, err = session.Exec(`truncate table user`)
	if err != nil {
		log.Fatalln(err)
	}

	_, err = session.Exec(`alter table user auto_increment=1`)
	if err != nil {
		log.Fatalln(err)
	}
