...
func TestMain(m *testing.M) {
	conn, err := sql.Open("mysql", "root:mysqlpw@tcp(127.0.0.1:55000)/test?charset=utf8mb4&parseTime=true&loc=Local")
	if err != nil {
		log.Fatalln(err)
	}
...
func TestUpdate(t *testing.T) {
	...
	t.Run("UpdateOrderByIdDescLimitCount", initAndRun(func(t *testing.T) {
		...
		newUser := mustMockUser()
		newUser.Id = mockUser.Id
		err = um.UpdateOrderByIdDescLimitCount(ctx, newUser, model.UpdateOrderByIdDescLimitCountWhereParameter{IdEqual: mockUser.Id})
		assert.NoError(t, err)
		actual, err := um.FindOne(ctx, model.FindOneWhereParameter{IdEqual: mockUser.Id})
		assert.NoError(t, err)
		assertUserEqual(t, newUser, actual)
...
func TestDelete(t *testing.T) {
	...
	t.Run("DeleteOneOrderByIDDescLimitCount", initAndRun(func(t *testing.T) {
		...
		err = um.DeleteOneOrderByIDDescLimitCount(ctx, model.DeleteOneOrderByIDDescLimitCountWhereParameter{NameEqual: list[0].Name})
		assert.NoError(t, err)
		actual, err := um.FindAll(ctx)
		assert.NoError(t, err)
