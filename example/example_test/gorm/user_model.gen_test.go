...
import (
	...
	model "github.com/anqiansong/sqlgen/example/gorm"
...
func TestMain(m *testing.M) {
	var err error
	conn := mysql.Open("root:mysqlpw@tcp(127.0.0.1:55000)/test?charset=utf8mb4&parseTime=true&loc=Local")
...
func mustInitDB(db *gorm.DB) {
	err := db.Transaction(func(tx *gorm.DB) error {
		tx.Exec(`SET SESSION sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY,',''))`)
		tx.Exec(`truncate table user`)
		tx.Exec(`alter table user auto_increment=1`)
		return nil
	})
	if err != nil {
		log.Fatal(err)
